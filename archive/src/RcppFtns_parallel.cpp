...
// [[Rcpp::export]]
vec compLam0_parallel(vec ts, double maxT, vec lam0m, vec indlam0, vec knts, int num){
  int n = ts.size(), m = lam0m.size()-1, dummy;
  vec lam0 = zeros(n);
...
// [[Rcpp::export]]
double compIntLam0_parallel(double maxT, vec lam0m, int num){
  int m = lam0m.size()-1;
  vec intLam0 = zeros(m);
...
// [[Rcpp::export]]
double compLogLiki_parallel(vec ts, double maxT, vec lam0m, vec indlam0, vec knts, double alpha, double eta, int num){
  int m = lam0m.size()-1, n = ts.size();
  double sumIntH;
  vec lam = zeros(n), intLam0 = zeros(m);
...
// [[Rcpp::export]]
vec compLogLik_parallel(vec ts, mat Xm, double maxT, vec indlam0, vec knts, mat beta, mat Wm, vec alpha, vec eta, int num){
  int niters = beta.n_rows, p = beta.n_cols, m = Wm.n_cols;
  vec loglik = zeros(niters), lam0m = zeros(m);
  ...
  for(int iter = 0; iter < niters; iter ++){
    ...
    loglik[iter] = compLogLiki_parallel(ts, maxT, lam0m, indlam0, knts, alpha[iter], eta[iter], num);
  }
...
// [[Rcpp::export]]
double comp_sumtdoffsp_parallel(vec ts, vec branching, uvec ind_offspring, int numoffsp, int num){
  ...
  if(numoffsp == 0){
    ...
    vec res = zeros(numoffsp);
    
#pragma omp parallel for num_threads(num) schedule(static)
    for (int i = 0; i < numoffsp; i++) {
      res[i] = ts[ind_offspring[i]] - ts[branching[ind_offspring[i]]-1]; // time difference between parent and offspring
    }
...
// [[Rcpp::export]]
vec comp_lam0z0_parallel(vec lam0, uvec dummy_background, int numbackground, int num){
  
  vec lam0z0 = zeros(numbackground);
...
// [[Rcpp::export]]
List fitNHPPSE_parallel(int niter, vec ts, mat Xm, double maxT, vec knts,
               vec beta, double alpha, double eta,
               vec indlam0,
               double shape_alpha, double rate_alpha, 
               double lb_eta, double ub_eta,
               vec sigma2, mat COVbeta, double COVeta,
               bool updateCOV, int adaptInterval, double adaptFactorExponent, vec adapIter, int num) {
  ...
  vec newlam0m, lam0, newlam0, lam0z0, newlam0z0, newbeta;
  double newalpha, neweta, intLam0, newintLam0;
  uvec ind_background, ind_offspring;
  vec rhat = zeros(2), gamma1 = zeros(2), gamma2 = zeros(2), dummyaccprob;
  double c0 = 1, c1 = adaptFactorExponent, ropt = 0.234, shape, scale;
  ...
  intLam0 = compIntLam0_parallel(maxT, lam0m, num);
  lam0 = compLam0_parallel(ts, maxT, lam0m, indlam0, knts, num);
  ...
  for(int s = 0; s < niter; s++) {
    ...
    ind_background = find( branching == 0 );
    ind_offspring = find( branching > 0 );
    numbackground = ind_background.size();
    numoffsp = n - numbackground;
    
    sumtdoffsp = comp_sumtdoffsp_parallel(ts, branching, ind_offspring, numoffsp, num);
    lam0z0 = comp_lam0z0_parallel(lam0, ind_background, numbackground, num);
    ...
    newlam0 = compLam0_parallel(ts, maxT, newlam0m, indlam0, knts, num);
    newintLam0 = compIntLam0_parallel(maxT, newlam0m, num);
    newlam0z0 = comp_lam0z0_parallel(newlam0, ind_background, numbackground, num);

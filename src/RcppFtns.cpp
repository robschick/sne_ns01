...
// ============================================================================-
// Compute background and excitation intensity
// ============================================================================-

// Compute posterior mean and CI of the intensity at time t 
// [[Rcpp::export]]
List compLamt(double t, vec ts, vec knts, double maxT, double indlam0t, 
              mat Xm, mat beta, vec delta, mat Wm, vec alpha, vec eta){
  
  int niters = beta.n_rows, m = Wm.n_cols, sizeind, n = ts.size();
  vec lam0m;
  vec mu = zeros(niters), se = zeros(niters);
  vec P = { 0.025, 0.5, 0.975 }, ahi;
  mat resBack, resSE, resLam;
  double alphaiter, etaiter, phiiter;
  uvec ind;
  
  // background rate
  for(int iter = 0; iter < niters; iter ++){
    lam0m = exp( Xm * trans(beta.row(iter)) + delta[iter] * trans(Wm.row(iter)) );
    mu[iter] = lam0m[indlam0t] + ( (lam0m[indlam0t+1] - lam0m[indlam0t]) / (maxT / m) ) * (t - knts[indlam0t]);  
  }
  resBack = quantile(mu, P);
  ...
  for(int iter = 0; iter < niters; iter ++){
    ...
    ahi = zeros(sizeind);
    for(int j = 0; j < sizeind; j ++){
      ahi[j] = alphaiter * exp( - (t - ts[j]) * etaiter );
